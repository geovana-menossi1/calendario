...
// Verificar se um evento deve ser excluído
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['excluir_evento']) && $_POST['excluir_evento'] == "1") {
    $evento_id = $mysqli->real_escape_string($_POST['id_evento']);
    $sql_excluir = "DELETE FROM eventos WHERE evento_id = ?";
    $stmt_excluir = $mysqli->prepare($sql_excluir);
    if ($stmt_excluir) {
        $stmt_excluir->bind_param('i', $evento_id);
...
$sql = "SELECT * FROM eventoNome WHERE email_usuario = ?";
...
<body>
    ...
    <main class="pai">
        <div class="conteudo">
            <div class="container-sm sas" style="width: 800px;">
                <div class="heading">Eventos Cadastrados</div>
                <?php
                if (count($result) > 0) {
                    echo '<table class="tabela-eventos">';
                    echo '<tr><th>Evento</th><th>Data</th><th>Descrição</th><th>Ações</th></tr>';

                    foreach ($result as $row) {
                        echo '<tr class="lista">';
                        echo '<td class="tarefa">' . htmlspecialchars($row["nome_evento"]) . '</td>';
                        echo '<td class="tarefa">' . htmlspecialchars($row["data_evento"]) . '</td>';
                        echo '<td class="tarefa">' . htmlspecialchars($row["descricao_evento"]) . '</td>';
                        echo '<td>
                                <form method="post" style="display:inline;">
                                    <input type="hidden" name="id_evento" value="' . $row["evento_id"] . '">
                                    <input type="hidden" name="excluir_evento" value="1">
                                    <input type="submit" value="Excluir" class="btn btn-danger">
                                </form>
                                <form method="get" action="update.php" style="display:inline;">
                                    <input type="hidden" name="id" value="' . $row["evento_id"] . '">
